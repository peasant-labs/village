...
func loadLegacyDispatchFixtures() ([]legacyDispatchCase, error) {
	...
	for _, name := range strings.Fields("sparse_legacy_envelope sparse_legacy_bare opaque_plaintext legacy_provider_native_usage legacy_jsonl legacy_array empty_harness_canonical_rewrite older_assistant_observation historical_nonassistant_observation invalid_older_observation pi_context_opaque pi_context_sparse_nonpi pi_context_no_identity explicit_pi_sparse legacy_key_pi_not_normalized metadata_null_is_presence metadata_empty_is_presence usage_null_is_presence usage_empty_is_presence source_ref_empty_is_presence tool_usage_null_is_presence call_ref_empty_is_presence result_ref_null_is_presence namespace_empty_requires_release namespace_null_requires_release malformed_json_not_opaque duplicate_escaped_legacy_key malformed_interior_jsonl public_envelope_mixed_jsonl public_bare_mixed_array recursively_wrapped_public_envelope wrong_harness_metadata old_role_plus_new_marker complete_reusable_nonpi_usage complete_nonassistant_observation_with_new_evidence complete_nullable_usage missing_discriminator_does_not_hide_new_fields") {
		if !seen[name] {
