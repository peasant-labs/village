...
// Session-context capture fixture (opt-in): a child session whose context
// source and starter resolve to two DISTINCT current targets, plus an
// uncertain earlier-history section. The durable relationships live on the
// content; the authorized navigation rides the metadata response, exactly as
// the real routes split them. Opt-in because the relationship fields force
...
if (CONTEXT_NAV) {
  ...
  // Refs make the payload strict-valid at the REST boundary, the same shape a
  // real graph publication serves.
  content.turns = content.turns.map((turn, i) => ({ ...turn, sourceEntryRef: `e_capture_${i}` }))
  content.relationships = [
    { kind: 'started_by', targetState: 'target_known', targetLocalId: '5a5a5a5a-0000-4000-8000-000000000002', evidence: 'native_typed' },
    {
      kind: 'context_from',
      targetState: 'target_known_retained',
      targetLocalId: 'c0c0c0c0-0000-4000-8000-000000000001',
      evidence: 'retained_last_good',
      anchor: { kind: 'through_redacted_entry', sourceEntryRef: 'e_parent', sourceRevisionRef: 'sha3-256:capture' },
    },
  ]
  content.earlierHistory = [
    {
      ...
      turns: [
        {
          index: 0,
          role: 'user',
          content: 'inherited earlier work, retained with uncertain ownership',
          timestamp: ts(-3),
          depth: 0,
        },
      ],
  ...
  contextNavigation = [
    { kind: 'started_by', status: 'resolved', transcriptId: starterTarget },
    {
      kind: 'context_from',
      status: 'resolved',
...
if (contextNavigation) detail.relationshipNavigation = contextNavigation
    ...
    return send(res, 200, {
      ...
      digest: {
        ...
        items: [
          ...
          // A long prompt carrying an unbroken token, so the capture shows the
          // clamp and the wrap rather than a row that happens to be short.
          { kind: 'prompt', transcriptId: '22222222-2222-2222-2222-222222222222', timestamp: '2026-06-28T12:03:00Z', text: 'please make the pull request page read the same way as the transcript viewer: the chain should show every session boundary, then each prompt in order, and each commit anchor with its change counts, and it must not overflow the panel when a prompt contains something long like https://github.com/peasant-labs/village/blob/0123456789abcdef0123456789abcdef01234567/frontend/src/app/pulls/%5Bowner%5D/%5Bname%5D/%5Bnumber%5D/page.tsx?plain=1#L120-L180 or a minified line of configuration, so clamp the preview and let the link carry the reader to the exact turn that produced it. This sentence repeats so the row is certainly longer than the clamp: the chain must stay complete while each prompt stays a readable preview, the chain must stay complete while each prompt stays a readable preview, the chain must stay complete while each prompt stays a readable preview, the chain must stay complete while each prompt stays a readable preview, the chain must stay complete while each prompt stays a readable preview, and the reader reaches the rest through the link.', ordinal: 3, turnIndex: 6 },
        ],
      },
      transcripts: [
